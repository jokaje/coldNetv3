...
def ffmpeg_normalize_to_wav(src_path: str, dst_path: str):
    """
    Normalisiert eine Quelldatei mit ffmpeg zu einer 16kHz Mono WAV-Datei.
    Diese Funktion wendet einen Dynamic Audio Normalizer und eine Lautstärkeanhebung an,
    um die Audioqualität für Spracherkennungsmodelle wie Whisper zu optimieren.
    """
    cmd = [
        "ffmpeg", "-y",
        "-i", src_path,
        "-af", "dynaudnorm=f=150:g=15,volume=3dB",
        "-ar", "16000",
        "-ac", "1",
        "-c:a", "pcm_s16le",
        dst_path
...
async def proxy_stt(
    ...
    # 1) Upload in eine temporäre Datei schreiben
    try:
    ...
    # 2) Nach 16 kHz Mono WAV normalisieren
    try:
    ...
    # 3) An den AI-Server (Whisper) schicken
    try:
...
async def create_chat_message(chat_id: int, payload: PromptPayload,
    ...
    ai_payload = {"role": "user", "content": payload.final_prompt}
    try:
        async with httpx.AsyncClient(timeout=60.0) as client:
...
def update_chat_details(chat_id: int, update_data: schemas.ChatUpdate,
    ...
    if update_data.is_pinned is not None and update_data.is_pinned:
        ...
        if pinned_count >= 5 and not chat.is_pinned:  # Only check if we are trying to pin a new one
